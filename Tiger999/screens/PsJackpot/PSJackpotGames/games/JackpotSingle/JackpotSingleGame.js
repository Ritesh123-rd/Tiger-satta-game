...
import { getWalletBalance, psJackpotMarket, JackpotSingleGame as JackpotSingleGameAPI } from '../../../../../api/auth';
...
export default function JackpotSingleGame({ navigation, route }) {
    ...
    const fetchMarketId = async () => {
        if (marketId) return;
        try {
            console.log('JackpotSingle Game: Fetching markets for gameName:', gameName);
    ...
    const finalSubmit = async () => {
        ...
        try {
            ...
            if (!marketId) {
                ...
                const markets = await psJackpotMarket();
    ...
    const submitWithId = async (userId, username, currentMarketId) => {
        try {
            ...
            console.log('JackpotSingleGame: Submitting bids. Payload:', {
                userId, username, numbers, amounts, gameName, currentMarketId, sessionTime
            });
            const response = await JackpotSingleGameAPI(
                userId,
                username,
                numbers,
                amounts,
