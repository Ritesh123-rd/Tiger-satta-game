...
import { getWalletBalance, placeHalfSangamBBet, getMarkets } from '../../../api/auth';
...
export default function HalfSangamBGame({ navigation, route }) {
    ...
    const { gameName, marketId, marketName, isOpenAvailable = true, isCloseAvailable = true } = route.params || { gameName: 'HALF SANGAM (B)' };
        ...
        try {
            ...
            if (response && (response.status === true || response.status === 'true')) {
                ...
                if (currentMarket) {
                    ...
                    console.log('Market fetched and updated for HalfSangamB:', currentMarket.id, currentMarket.market_name);
                }
            }
        } catch (error) {
    ...
    const handleAddBid = () => {
        if (!openAnk || openAnk.length !== 1) {
            Alert.alert('Error', 'Please enter valid Open Ank (1 digit)');
            return;
        }
    ...
    const handleConfirmSubmit = async () => {
        ...
        try {
            ...
            const response = await placeHalfSangamBBet(
                userId,
                username,
                formattedBids,
                marketName || gameName,
