...
export default function JodiFamilyGame({ navigation, route }) {
    ...
    const handleSubmit = () => {
        if (!digit || digit.length !== 1) { Alert.alert('Error', 'Please enter valid digit (0-9)'); return; }
        if (!points || parseInt(points) <= 0) { Alert.alert('Error', 'Please enter valid points'); return; }
        Alert.alert('Success', `Jodi Family bet placed for digit ${digit} with ${points} points!`, [{ text: 'OK', onPress: () => { setDigit(''); setPoints(''); } }]);
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <MarqueeText text={`${gameName} - JODI FAMILY`} style={styles.headerTitle} />
            ...
            <ScrollView style={styles.content}>
                ...
                <View style={styles.inputGroup}>
                    <Text style={styles.inputLabel}>Enter Digit (0-9):</Text>
                    <TextInput style={styles.textInput} placeholder="0" placeholderTextColor="#999" keyboardType="numeric" maxLength={1} value={digit} onChangeText={setDigit} />
                </View>
