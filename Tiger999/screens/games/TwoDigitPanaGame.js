...
export default function TwoDigitPanaGame({ navigation, route }) {
  ...
  const handleSubmit = () => {
    if (!pana || pana.length !== 2) { Alert.alert('Error', 'Please enter valid 2-digit pana'); return; }
    if (!points || parseInt(points) <= 0) { Alert.alert('Error', 'Please enter valid points'); return; }
    Alert.alert('Success', `Two Digit Pana bet placed for ${pana} with ${points} points!`, [{ text: 'OK', onPress: () => { setPana(''); setPoints(''); } }]);
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>Enter 2-Digit Pana:</Text>
          <TextInput style={styles.textInput} placeholder="00" placeholderTextColor="#999" keyboardType="numeric" maxLength={2} value={pana} onChangeText={setPana} />
        </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 14, fontWeight: 'bold', color: '#000', fontFamily: 'RaleighStdDemi', textTransform: 'uppercase' },
