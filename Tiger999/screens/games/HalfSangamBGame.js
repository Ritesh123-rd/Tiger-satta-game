...
export default function HalfSangamBGame({ navigation, route }) {
    const { gameName } = route.params || { gameName: 'HALF SANGAM (B)' };
    ...
    const handleSubmit = () => {
        ...
        if (!closeDigit || closeDigit.length !== 1) { Alert.alert('Error', 'Please enter valid Close Digit (1 digit)'); return; }
        if (!points || parseInt(points) <= 0) { Alert.alert('Error', 'Please enter valid points'); return; }
        Alert.alert('Success', `Half Sangam (B) bet placed: ${openPana}-${closeDigit} for ${points} points!`, [{ text: 'OK', onPress: () => { setOpenPana(''); setCloseDigit(''); setPoints(''); } }]);
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content}>
                ...
                <View style={styles.inputGroup}>
                    <Text style={styles.inputLabel}>Enter Close Digit:</Text>
                    <TextInput style={styles.textInput} placeholder="0" placeholderTextColor="#999" keyboardType="numeric" maxLength={1} value={closeDigit} onChangeText={setCloseDigit} />
                </View>
