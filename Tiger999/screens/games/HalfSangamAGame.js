...
export default function HalfSangamAGame({ navigation, route }) {
  ...
  useFocusEffect(
    ...
    const { gameName } = route.params || { gameName: 'HALF SANGAM (A)' };
    ...
    const handleSubmit = () => {
        if (!openDigit || openDigit.length !== 1) { Alert.alert('Error', 'Please enter valid Open Digit (1 digit)'); return; }
        ...
        Alert.alert('Success', `Half Sangam (A) bet placed: ${openDigit}-${closePana} for ${points} points!`, [{ text: 'OK', onPress: () => { setOpenDigit(''); setClosePana(''); setPoints(''); } }]);
    };
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content}>
                ...
                <View style={styles.inputGroup}>
                    <Text style={styles.inputLabel}>Enter Open Digit:</Text>
                    <TextInput style={styles.textInput} placeholder="0" placeholderTextColor="#999" keyboardType="numeric" maxLength={1} value={openDigit} onChangeText={setOpenDigit} />
                </View>
